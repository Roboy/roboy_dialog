            ...
            try {
              ...
              if (opts.allowedRange != null) {
                if (numberValue.value < opts.allowedRange.get(0) || numberValue.value > opts.allowedRange.get(1)) {
                  LogInfoToggle.warnings("NumberFn: %f is outside of the allowed range %s", numberValue.value, opts.allowedRange);
                  return null;
                }
              }
              ...
              LogInfoToggle.warnings("NumberFn: Cannot convert NerSpan \"%s\" to a number", value);
            }
          }
        }
