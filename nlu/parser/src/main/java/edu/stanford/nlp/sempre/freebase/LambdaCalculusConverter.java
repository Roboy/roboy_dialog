...
public class LambdaCalculusConverter implements Runnable {
  ...
  public void readPrereqs() {
    LogInfoToggle.begin_track("Reading prereq");
    readSpec();
    readPrimitives();
    readVars();
  ...
  Formula toOrFormula(List<LispTree> clauses, boolean[] hit, String headVar, List<String> existsVars) {
    ...
    LogInfoToggle.log("# or clauses: " + clauses.size());
    Formula formula = null;
    for (int i = 0; i < clauses.size(); i++) {
      if (hit[i]) {
        LogInfoToggle.log("hit " + i);
    ...
    try {
      ...
      while ((line = in.readLine()) != null) {
        ...
        if (!gotUtterance) {
          ...
          LogInfoToggle.logs(color.colorize("IN [%d]: %s", "blue"), newId, utterance);
          LogInfoToggle.logs(color.colorize("IN [%d]: %s", "purple"), newId, tree);
          ArrayList<String> existsVars = new ArrayList<String>();

          if (manualConversionsMap.containsKey(line)) {
            LogInfoToggle.logs("MANUAL CONVERSION=%s", line);
            ex.setTargetFormula(Formula.fromString(manualConversionsMap.get(line)));
          } else {
            LogInfoToggle.log("AUTOMATIC CONVERSION");
            ex.setTargetFormula(toFormula(tree, null, existsVars));
          }
          LogInfoToggle.logs(color.colorize("OUT [%d]: %s", "yellow"), newId, ex.createExample().targetFormula.toLispTree());
  ...
  void executeExamples() {
    ...
    for (Example ex : examples) {
      ...
      LogInfoToggle.logs(color.colorize("[%d] %s", "blue"), exInd, ex.utterance);
      LogInfoToggle.logs(color.colorize("[%d] %s", "yellow"), exInd, ex.targetFormula.toString());
      try {
        Executor.Response response = executor.execute(ex.targetFormula, null);
        LogInfoToggle.logs("\t\t [%d] %s", exInd, response.value.toString());
        validExampleIds.add(exInd);
        validExampleLengths.add(ex.utterance.length());
      } catch (RuntimeException e) {
        LogInfoToggle.error(e);
...
//    });
  ...
  void printSummary() {
    LogInfoToggle.logs("%d input examples", examples.size());
    LogInfoToggle.logs("%d successful executions", validExampleIds.size());
    LogInfoToggle.logs("Failed executions (%d):", failedExampleIds.size());
    for (int k : failedExampleIds) {
      Example ex = examples.get(k - 1);
      LogInfoToggle.log(ex.toJson());
    }
  }
