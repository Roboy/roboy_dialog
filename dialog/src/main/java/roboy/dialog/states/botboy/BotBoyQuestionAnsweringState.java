 ...
 *
 *
 * BotBoyQuestionAnsweringState interface:
...
public class BotBoyQuestionAnsweringState extends State {
    ...
    public BotBoyQuestionAnsweringState(String stateIdentifier, StateParameters params) {
    ...
    private boolean isIntentsHistoryComplete(Neo4jRelationship[] predicates) {
        ...
        for (Neo4jRelationship predicate : predicates) {
            if (!getContext().DIALOG_INTENTS.contains(new IntentValue(BotBoyPersonalInformationFollowUpState.INTENTS_HISTORY_ID, predicate))) {
                isComplete = false;
            }
        }
        return isComplete;
